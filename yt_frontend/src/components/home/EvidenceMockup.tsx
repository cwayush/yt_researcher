...
import { EVIDENCE_EXAMPLE } from "@/data/content";

const { question, answer, timestamp, quote } = EVIDENCE_EXAMPLE;
...
export function EvidenceMockup() {
  return (
    <div className="rounded-panel border-cream/12 bg-cream/6 border p-6">
      ...
      <p className="text-cream/75 mb-5 text-sm leading-relaxed">{answer}</p>
      ...
      <div className="rounded-card border-cream/12 bg-cream/6 border p-4">
        <div className="mb-2 flex items-center justify-between">
          ...
          <span className="font-mono-ts text-gold/90 text-xs">{timestamp}</span>
        </div>
